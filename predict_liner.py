...
def main():
    dataset = generate_dataset(80,type="liner")
    support_vectors = find_support_vectors(dataset,  C=0.5, slack=0.5)
    alfa = learn_alfa(support_vectors)
    ...
    weight = get_weight(support_vectors,alfa)
    basis = get_basis(support_vectors, weight)
    # plt.arrow(0,0,weight[0], weight[1], head_width=0.05, head_length=0.1, label="Weight", color='blue')

    # Hyperline
    x = [-2, -1, 0, 1, 2, 3, 4, 5]
    plt.plot(x, get_axis(x, weight, basis), label="Hyperline")

    # Prediction
    for i in range(20):
        rx = float(randint(0, 8000))
        ry = float(randint(0, 8000))
        x = [5 - rx / 1000, 5 - ry / 1000]
        r = predict(support_vectors, alfa, x, basis=basis)
